...
tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( dsyr.length, 1, 'has expected arity' );
	t.end();
});
...
tape( 'the function performs the symmetric rank 1 operation `A = alpha*x*x^T + A` (upper)', function test( t ) {
	...
	v = dsyr( [ x, A, uplo, alpha ] );

	expected = new Float64Array( [ 2.0, 4.0, 6.0, 2.0, 5.0, 8.0, 3.0, 2.0, 10.0 ] );
	...
	expected = new Float64Array( [ 2.0, 2.0, 3.0, 3.0, 2.0, 2.0, 4.0, 3.0, 2.0 ] );
	t.strictEqual( v, A, 'returns expected value' );
	t.strictEqual( isSameFloat64Array( getData( v ), expected ), true, 'returns expected value' );
...
tape( 'the function performs the symmetric rank 1 operation `A = alpha*x*x^T + A` (lower)', function test( t ) {
	...
	expected = new Float64Array( [ 2.0, 2.0, 3.0, 4.0, 5.0, 2.0, 6.0, 8.0, 10.0 ] );
	t.strictEqual( v, A, 'returns expected value' );
	t.strictEqual( isSameFloat64Array( getData( v ), expected ), true, 'returns expected value' );
...
tape( 'the function supports ndarrays having non-unit strides', function test( t ) {
	...
	expected = new Float64Array( [ 1.0, 2.0, 3.0, 0.0, 4.0, 6.0, 0.0, 0.0, 9.0 ] );
	t.strictEqual( v, A, 'returns expected value' );
	t.strictEqual( isSameFloat64Array( getData( v ), expected ), true, 'returns expected value' );
	t.end();
});
